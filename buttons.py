...
class Button:
    ...
    def __init__(self,name,(x1,y1),centre=True,caption=None,colour=None): # eg ('plus',(30,40))
        self._instances.append(self)
        t=name
        if colour!=None: t=colour
        ...
        if caption !=None:
            self.caption=g.font3.render(caption,True,(0,0,255))
...
def xy(name):
    for b in Button._instances:
        if b.name==name: return (b.x,b.y) #****
    return None # not found

def w(name):
    for b in Button._instances:
        if b.name==name: return b.w #****
    return None # not found
